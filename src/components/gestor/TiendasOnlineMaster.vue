  ...
  <div class="col-md-12 mx-auto">
    ...
    >
      <div class="row">
        ...
        <div class="col-md-2">
          <strong>Códigos Postales: </strong>{{ tienda.codPostal.join(", ") }}
        </div>
        <div class="col-md-2"><strong>Envío: </strong>{{ tienda.envio }} €</div>
      </div>
      <div class="row">
        <div class="col-md-6">
...
import EditarTienda from "./TiendasOnlineEditar.vue";
...
export default defineComponent({
  ...
  computed: {
    tiendasFiltradas() {
      let tiendas = [...this.tiendasOnlineStore.tiendas];
      ...
      if (this.filtroCodPostal) {
        tiendas = tiendas.filter((tienda) =>
          tienda.codPostal.some((cp) => cp.includes(this.filtroCodPostal))
        );
      }
  ...
  methods: {
    buscarIndice(tienda) {
      return this.tiendasOnlineStore.buscarIndiceTienda(tienda.codTienda);
    },
    editarTienda(index) {
      this.tiendaEditando = this.tiendasOnlineStore.tiendas[index];
      this.editandoTienda = true;
    },
    eliminarTienda(index) {
      this.showModal = true;
      this.tiendaSeleccionada = this.tiendasOnlineStore.tiendas[index];
      this.indiceTienda = index;
    },
    confirmarEliminar() {
      this.tiendasOnlineStore.eliminarTienda(this.indiceTienda);
